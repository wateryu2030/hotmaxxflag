...
**无需打开 Cursor**，双击即可启动看板，且**锁屏后仍可访问**（看板以后台服务运行）：

1. 在项目根目录双击 **`启动好特卖看板.command`**，或  
2. 先双击 **`创建桌面快捷方式.command`**，再在桌面双击「启动好特卖看板」快捷方式。

首次运行会安装系统后台服务（LaunchAgent），之后每次双击会启动服务并打开浏览器。关闭终端窗口不影响看板运行。
