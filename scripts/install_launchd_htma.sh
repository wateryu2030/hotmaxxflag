...
ENV_KEYS="HTMA_PUBLIC_URL FEISHU_APP_ID FEISHU_APP_SECRET FLASK_SECRET_KEY MYSQL_HOST MYSQL_PORT MYSQL_USER MYSQL_PASSWORD MYSQL_DATABASE HTMA_SUPER_ADMIN_OPEN_ID FEISHU_WEBHOOK_URL OPENCLAW_GATEWAY_URL OPENCLAW_GATEWAY_TOKEN WECHAT_APPID WECHAT_APPSECRET WECHAT_JWT_SECRET WECHAT_MINI_ENABLED HTMA_SKIP_MOBILE_JWT HTMA_STORE_ID HTMA_LLM_PROVIDER HTMA_LLM_API_URL HTMA_LLM_API_KEY HTMA_LLM_MODEL OPENAI_API_KEY OPENAI_API_BASE OPENAI_MODEL DEEPSEEK_API_KEY DEEPSEEK_MODEL HTMA_DEEPSEEK_API_BASE DOUBAO_API_KEY DOUBAO_MODEL DOUBAO_API_BASE VOLCANO_ENGINE_API_KEY HTMA_CRON_WEBHOOK_URL HTMA_CRON_NOTIFY_SUCCESS"
ENV_XML=""
...
#!/bin/bash
# 默认 HTTP/2 连边缘，减少 QUIC 在部分网络下超时导致 ERR_CONNECTION_CLOSED（可设 TUNNEL_TRANSPORT_PROTOCOL=quic 覆盖）
