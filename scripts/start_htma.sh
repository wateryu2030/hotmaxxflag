...
# 确保表结构完整
python "$ROOT/scripts/run_add_columns.py" 2>/dev/null || true
...
python "$ROOT/scripts/check_feishu_env.py" 2>/dev/null || true
...
cd "$ROOT/htma_dashboard" && exec env FEISHU_APP_ID="${FEISHU_APP_ID}" FEISHU_APP_SECRET="${FEISHU_APP_SECRET}" HTMA_PUBLIC_URL="${HTMA_PUBLIC_URL}" python -u app.py
